...
export function HRDataImportCenter() {
  ...
  const validateAndPreviewOrg = (data: any[]) => {
    ...
    data.forEach((row, index) => {
      ...
      if (row['部门编码'] && row['部门名称']) {
        validData.push({
          ...
          name: row['部门名称'],
          code: row['部门编码'],
          parentId: row['上级部门编码'] || null,
          type: row['部门类型'] || 'department',
    ...
    if (errors.length === 0) {
      toast.success(`成功解析${validData.length}个部门`);
      setCurrentStep(2);
    } else {
      toast.warning(`发现${errors.length}个问题，请修正后重新上传`);
    }
  };
  ...
  const validateAndPreviewEmployees = (data: any[]) => {
    ...
    data.forEach((row, index) => {
      ...
      if (row['角色'] && !validRoles.includes(row['角色'])) {
        errors.push(`第${lineNum}行：角色必须是 employee/manager/gm/hr 之一`);
      }
      ...
      if (row['员工编号'] && row['姓名']) {
        validData.push({
          ...
          name: row['姓名'],
          department: row['部门编码'],
          subDepartment: row['部门编码'],
          position: row['岗位'] || '员工',
          role: row['角色'] || 'employee',
          level: row['职级'] || 'intermediate',
          managerId: row['直属上级编号'] || null,
          joinDate: row['入职日期'] || null,
          phone: row['手机号'] || '',
          email: row['邮箱'] || ''
        });
      }
    ...
    if (errors.length === 0) {
      toast.success(`成功解析${validData.length}名员工`);
      setCurrentStep(2);
    } else {
      toast.warning(`发现${errors.length}个问题，请修正后重新上传`);
    }
  ...
  const buildOrgTree = useCallback(() => {
    ...
    const deptMap = new Map<string, OrgTreeNode>(
      departments.map(d => [d.id, { ...d, children: [], employees: [] }])
    );
    ...
    employees.forEach(emp => {
      const dept = deptMap.get(emp.subDepartment || emp.department);
      if (dept) {
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="template" className="space-y-4">
              ...
              <CardContent className="space-y-4">
                <div className="bg-gray-50 p-4 rounded-lg text-sm text-gray-600 space-y-2">
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li>部门名称</li>
                    <li>上级部门编码（用于层级）</li>
                    <li>部门类型（company/department/subDepartment）</li>
                    <li>排序</li>
                  </ul>
                </div>
                <Button onClick={() => downloadTemplate('org')} className="w-full">
              ...
              <CardContent className="space-y-4">
                <div className="bg-gray-50 p-4 rounded-lg text-sm text-gray-600 space-y-2">
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li>姓名</li>
                    <li>部门编码（关联组织架构）</li>
                    <li>角色（employee/manager/gm/hr）</li>
                    <li>职级（senior/intermediate/junior/assistant）</li>
                    <li>直属上级编号（用于考核关系）</li>
                    <li>入职日期、手机号、邮箱</li>
                  </ul>
                </div>
                <Button onClick={() => downloadTemplate('employee')} className="w-full">
                  <Download className="w-4 h-4 mr-2" />
                  下载员工名册模板
                </Button>
              </CardContent>
          ...
          <div className="flex justify-end">
            <Button onClick={() => { setActiveTab('org'); setCurrentStep(1); }}>
              下一步：设置组织架构
        ...
        <TabsContent value="org" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>导入组织架构</CardTitle>
              <CardDescription>
                上传组织架构CSV文件，系统将自动解析部门层级关系
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {departments.length > 0 && (
                <div className="mt-6">
                  <h4 className="font-semibold mb-3">现有部门结构</h4>
                  <div className="border rounded-lg p-4 max-h-64 overflow-y-auto">
                    <OrgTree nodes={buildOrgTree()} />
                  </div>
                </div>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>导入员工名册</CardTitle>
              <CardDescription>
                上传员工名册CSV文件，系统将自动建立考核关系
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-500 transition-colors">
                ...
                <p className="text-sm text-gray-500 mb-4">支持 CSV 格式，文件大小不超过 10MB</p>
                <input
                  type="file"
                  accept=".csv"
          ...
          <div className="flex justify-between">
            <Button variant="outline" onClick={() => { setActiveTab('template'); setCurrentStep(0); }}>
              <ChevronLeft className="w-4 h-4 mr-2" />
              上一步
            </Button>
            <Button 
              onClick={() => { setActiveTab('preview'); setCurrentStep(2); }}
              disabled={previewData.departments.length === 0 && previewData.employees.length === 0}
            >
              下一步：预览数据
              <ChevronRight className="w-4 h-4 ml-2" />
            </Button>
        ...
        <TabsContent value="preview" className="space-y-4">
            ...
            <Card className="border-red-200 bg-red-50">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-red-700">
                  ...
                  发现 {previewData.errors.length} 个问题
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          {previewData.departments.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Building2 className="w-5 h-5" />
                  待导入部门 ({previewData.departments.length}个)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
          ...
          {previewData.employees.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Users className="w-5 h-5" />
                  待导入员工 ({previewData.employees.length}人)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
          ...
          <div className="flex justify-between">
            <Button variant="outline" onClick={() => { setActiveTab('org'); setCurrentStep(1); }}>
              <ChevronLeft className="w-4 h-4 mr-2" />
              上一步
            </Button>
            <Button 
              onClick={() => { setActiveTab('import'); setCurrentStep(3); }}
              disabled={previewData.errors.length > 0 || (previewData.departments.length === 0 && previewData.employees.length === 0)}
            >
              下一步：执行导入
              <ChevronRight className="w-4 h-4 ml-2" />
            </Button>
        ...
        <TabsContent value="import" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                请确认以下数据将导入系统，导入后可以在员工管理和组织架构中查看
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg text-center">
                  <p className="text-3xl font-bold text-blue-600">{previewData.departments.length}</p>
                  <p className="text-sm text-gray-600 mt-1">待导入部门</p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg text-center">
                  <p className="text-3xl font-bold text-green-600">{previewData.employees.length}</p>
                  <p className="text-sm text-gray-600 mt-1">待导入员工</p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg text-center">
                  <p className="text-3xl font-bold text-purple-600">
                    {previewData.employees.filter(e => e.managerId).length}
                  </p>
                  <p className="text-sm text-gray-600 mt-1">考核关系数</p>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                ...
                <ul className="text-sm text-yellow-700 space-y-1">
                  <li>• 部门数据将先导入，建立组织架构</li>
                  <li>• 员工数据后导入，自动关联到对应部门</li>
                  <li>• 考核关系根据"直属上级编号"自动建立</li>
                  <li>• 已存在的数据（按编号判断）将被跳过</li>
                  <li>• 所有新员工默认密码为：123456</li>
                </ul>
              </div>
          ...
          <div className="flex justify-start">
            <Button variant="outline" onClick={() => { setActiveTab('preview'); setCurrentStep(2); }}>
              <ChevronLeft className="w-4 h-4 mr-2" />
              返回预览
            </Button>
          </div>
      ...
      <Dialog open={showOrgChart} onOpenChange={setShowOrgChart}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>组织架构与考核关系图</DialogTitle>
            <DialogDescription>
              可视化展示部门层级和上下级考核关系
            </DialogDescription>
          </DialogHeader>
...
// 组织架构树组件
function OrgTree({ nodes, level = 0 }: { nodes: any[]; level?: number }) {
  return (
...
// 考核关系组织图
function AssessmentOrgChart({ departments, employees }: { departments: Department[]; employees: Employee[] }) {
  // 构建层级结构
  const buildHierarchy = () => {
    const managers = employees.filter(e => e.role === 'manager' || e.role === 'gm');

    return managers.map(manager => {
      const subordinates = employees.filter(e => e.managerId === manager.id);
      return {
        manager,
        subordinates,
          ...
          <div className="flex items-center gap-3 mb-4 pb-3 border-b">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {department?.name || manager.department} · 
                {manager.role === 'manager' ? '部门经理' : '总经理'}
              </p>
            </div>
            <Badge className="ml-auto">
              考核 {subordinates.length} 人
            </Badge>
          </div>
