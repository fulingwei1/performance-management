...
export function StrategicGoalsDisplay({ compact = false, showDepartment = true }: StrategicGoalsDisplayProps) {
  ...
  const [companyStrategy, setCompanyStrategy] = useState<StrategicGoal | null>(null);
  ...
  const fetchStrategicGoals = async () => {
    try {
      ...
      // 分类
      const strategy = goals.find(g => g.type === 'company-strategy' && g.status === 'active');
      const keyWorks = goals.filter(g => g.type === 'company-key-work' && g.status === 'active');
      const deptWorks = goals.filter(g => 
        g.type === 'department-key-work' && 
        g.status === 'active' && 
        g.department === user?.department
      );

      setCompanyStrategy(strategy || null);
        ...
        <CardContent className="space-y-3 text-sm">
          {companyStrategy && (
            <div>
              ...
              <p className="text-blue-700 text-xs">{companyStrategy.description || companyStrategy.title}</p>
            </div>
          )}
          ...
          {showDepartment && departmentKeyWorks.length > 0 && (
            <div>
              <p className="font-medium text-green-900 mb-1">{user?.department} 重点工作</p>
              <ul className="text-xs text-green-700 space-y-1">
                {departmentKeyWorks.slice(0, 2).map(dw => (
                  <li key={dw.id}>• {dw.title}</li>
                ))}
              </ul>
            </div>
          )}

          {!companyStrategy && companyKeyWorks.length === 0 && departmentKeyWorks.length === 0 && (
        ...
        <CardContent>
          {companyStrategy ? (
            <div className="space-y-2">
              <h3 className="font-semibold text-blue-900">{companyStrategy.title}</h3>
              {companyStrategy.description && (
                <p className="text-sm text-blue-700">{companyStrategy.description}</p>
              )}
              ...
              <ul className="space-y-2">
                {departmentKeyWorks.map((dw, idx) => (
                  <li key={dw.id} className="flex gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-green-900">{dw.title}</p>
                      {dw.description && (
