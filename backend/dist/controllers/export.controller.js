...
exports.exportController = {
                    ...
                    if (includeAnalysis === 'true') {
                        ...
                        const analysisData = Array.from(deptStats.entries()).map(([dept, records]) => {
                            ...
                            return {
                                ...
                                '总人数': records.length,
                                '已评分人数': scores.length,
                                '平均分': scores.length > 0 ? formatScore(scores.reduce((a, b) => a + b, 0) / scores.length) : 0,
    ...
    exportAnnualPerformance: [
        (0, errorHandler_1.asyncHandler)(async (req, res) => {
            ...
            try {
                const allRecords = await performance_model_1.PerformanceModel.findAll();
                const yearRecords = allRecords.filter((record) => record.month.startsWith(year));
                const employees = await employee_model_1.EmployeeModel.findAll();
                const employeeMap = new Map();
                employees.forEach((emp) => employeeMap.set(emp.id, emp));
                ...
                yearRecords.forEach((record) => {
                    ...
                    if (record.totalScore > 0) {
                        ...
                        summary['评分次数']++;
                        if (record.totalScore >= 1.2)
                            summary['优秀次数']++;
                        else if (record.totalScore >= 1.0)
                            summary['良好次数']++;
                        else if (record.totalScore >= 0.8)
                            summary['合格次数']++;
                        else
                            summary['待改进次数']++;
                    }
                ...
                const finalData = Array.from(employeeSummary.values()).map(summary => {
                    ...
                    if (scores.length > 0) {
                        summary['平均分'] = formatScore(scores.reduce((a, b) => a + b, 0) / scores.length);
    ...
    exportEmployees: [
        (0, errorHandler_1.asyncHandler)(async (req, res) => {
            ...
            try {
                let employees = await employee_model_1.EmployeeModel.findAll();
