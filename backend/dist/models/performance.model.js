...
class PerformanceModel {
    ...
    static async findById(id) {
        if (database_1.USE_MEMORY_DB) {
            const record = database_1.memoryDB.performanceRecords.findById(id);
            if (!record)
                return null;
    ...
    static async findByEmployeeId(employeeId) {
        if (database_1.USE_MEMORY_DB) {
            const records = database_1.memoryDB.performanceRecords.findByEmployeeId(employeeId);
    ...
    // 获取经理的所有评分记录（下属）
    static async findByAssessorId(assessorId, month) {
        if (database_1.USE_MEMORY_DB) {
            // 获取经理的所有下属
            const subordinates = await employee_model_1.EmployeeModel.findByManagerId(assessorId);
            const subordinateIds = subordinates.map(s => s.id);
            // 获取这些员工的绩效记录
            const allRecords = Array.from(database_1.memoryStore.performanceRecords.values());
    ...
    static async findByMonth(month) {
        if (database_1.USE_MEMORY_DB) {
            const records = database_1.memoryDB.performanceRecords.findByMonth(month);
            return Promise.all(records.map((r) => this.enrichRecord(r)));
        }
        const sql = `
      SELECT 
    ...
    // 根据员工ID和月份查找记录
    static async findByEmployeeIdAndMonth(employeeId, month) {
        if (database_1.USE_MEMORY_DB) {
            const record = database_1.memoryDB.performanceRecords.findAll().find((r) => r.employeeId === employeeId && r.month === month);
    ...
    static async saveSummary(data) {
        ...
        if (database_1.USE_MEMORY_DB) {
            database_1.memoryDB.performanceRecords.create(record);
            return this.enrichRecord(record);
        }
        const sql = `
    ...
    static async submitScore(data) {
        if (database_1.USE_MEMORY_DB) {
            const record = database_1.memoryDB.performanceRecords.findById(data.id);
            if (!record)
                return null;
            const updated = database_1.memoryDB.performanceRecords.update(data.id, {
                ...data,
                status: 'completed',
                updatedAt: new Date()
            });
    ...
    // 更新排名
    static async updateRanks(month) {
        if (database_1.USE_MEMORY_DB) {
            const allRecords = Array.from(database_1.memoryStore.performanceRecords.values());
            ...
            for (const groupKey of deptGroups) {
                ...
                const groupRecords = records
                    .filter(r => r.subDepartment === subDept && r.groupType === groupType)
                    .sort((a, b) => b.totalScore - a.totalScore);
                groupRecords.forEach((record, index) => {
                    const r = database_1.memoryStore.performanceRecords.get(record.id);
                    if (r) {
                        r.groupRank = index + 1;
                        database_1.memoryStore.performanceRecords.set(record.id, r);
                    }
                });
            }
            // 跨部门同级别排名
        ...
        // Postgres版本
        await (0, database_1.transaction)(async (connection) => {
            // 1. 按部门+分组计算组内排名
    ...
    static async delete(id) {
        if (database_1.USE_MEMORY_DB) {
            return database_1.memoryDB.performanceRecords.delete(id);
        }
    ...
    // 按月份删除记录（返回删除数量）
    static async deleteByMonth(month) {
        if (database_1.USE_MEMORY_DB) {
            const idsToDelete = [];
            for (const [id, record] of database_1.memoryStore.performanceRecords.entries()) {
                if (record.month === month)
                    idsToDelete.push(id);
            }
            idsToDelete.forEach((id) => database_1.memoryStore.performanceRecords.delete(id));
            return idsToDelete.length;
        }
        const sql = 'DELETE FROM performance_records WHERE month = ?';
        const result = await (0, database_1.query)(sql, [month]);
        return result.affectedRows || 0;
    }
    // 删除全部记录（返回删除数量）
    static async deleteAll() {
        if (database_1.USE_MEMORY_DB) {
            const count = database_1.memoryStore.performanceRecords.size;
            database_1.memoryStore.performanceRecords.clear();
            return count;
        }
    ...
    static async enrichRecord(record) {
        const employee = await employee_model_1.EmployeeModel.findById(record.employeeId);
        const assessor = await employee_model_1.EmployeeModel.findById(record.assessorId);
        return {
            ...record,
