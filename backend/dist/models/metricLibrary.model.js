...
exports.MetricLibraryModel = void 0;
const database_1 = require("../config/database");
class MetricLibraryModel {
    // ============ 指标管理 ============
    // 获取所有指标
    static async findAllMetrics() {
        if (database_1.USE_MEMORY_DB) {
            return Array.from(database_1.memoryStore.performanceMetrics?.values() || []);
        }
        const sql = `
      SELECT 
        m.*,
    ...
    // 根据ID获取指标
    static async findMetricById(id) {
        if (database_1.USE_MEMORY_DB) {
            return database_1.memoryStore.performanceMetrics?.get(id) || null;
        }
        const sql = `
      SELECT 
        m.*,
    ...
    // 根据分类获取指标
    static async findMetricsByCategory(category) {
        if (database_1.USE_MEMORY_DB) {
            return Array.from(database_1.memoryStore.performanceMetrics?.values() || [])
      ...
      ORDER BY m.name
    `;
        const results = await (0, database_1.query)(sql, [category]);
        return results.map(this.formatMetric);
    }
    // 创建指标
    static async createMetric(metric) {
        const now = new Date().toISOString();
        const newMetric = { ...metric, createdAt: now, updatedAt: now };
        if (database_1.USE_MEMORY_DB) {
            if (!database_1.memoryStore.performanceMetrics)
                database_1.memoryStore.performanceMetrics = new Map();
            database_1.memoryStore.performanceMetrics.set(metric.id, newMetric);
            return newMetric;
        }
        const sql = `
    ...
    `;
        ...
        if (metric.scoringCriteria && metric.scoringCriteria.length > 0) {
            for (const criterion of metric.scoringCriteria) {
                await (0, database_1.query)('INSERT INTO scoring_criteria (id, metric_id, level, score, description) VALUES (?, ?, ?, ?, ?)', [`${metric.id}-${criterion.level}`, metric.id, criterion.level, criterion.score, criterion.description]);
            }
        }
        // 插入适用部门
        if (metric.departmentIds && metric.departmentIds.length > 0) {
            for (const deptId of metric.departmentIds) {
                await (0, database_1.query)('INSERT INTO metric_departments (metric_id, department_id) VALUES (?, ?)', [metric.id, deptId]);
            }
        }
        // 插入适用岗位
        if (metric.positionIds && metric.positionIds.length > 0) {
            for (const posId of metric.positionIds) {
                await (0, database_1.query)('INSERT INTO metric_positions (metric_id, position_id) VALUES (?, ?)', [metric.id, posId]);
            }
        }
        // 插入适用级别
        if (metric.applicableLevels && metric.applicableLevels.length > 0) {
            for (const level of metric.applicableLevels) {
                await (0, database_1.query)('INSERT INTO metric_levels (metric_id, level) VALUES (?, ?)', [metric.id, level]);
            }
        }
        return newMetric;
    }
    // 更新指标
    static async updateMetric(id, updates) {
        if (database_1.USE_MEMORY_DB) {
            const existing = database_1.memoryStore.performanceMetrics?.get(id);
            if (!existing)
                return null;
            const updated = { ...existing, ...updates, updatedAt: new Date().toISOString() };
            database_1.memoryStore.performanceMetrics?.set(id, updated);
            return updated;
        }
        const allowedFields = [
        ...
        if (updates.departmentIds) {
            await (0, database_1.query)('DELETE FROM metric_departments WHERE metric_id = ?', [id]);
            for (const deptId of updates.departmentIds) {
                await (0, database_1.query)('INSERT INTO metric_departments (metric_id, department_id) VALUES (?, ?)', [id, deptId]);
            }
        }
        if (updates.positionIds) {
            await (0, database_1.query)('DELETE FROM metric_positions WHERE metric_id = ?', [id]);
            for (const posId of updates.positionIds) {
                await (0, database_1.query)('INSERT INTO metric_positions (metric_id, position_id) VALUES (?, ?)', [id, posId]);
            }
        }
        if (updates.applicableLevels) {
            await (0, database_1.query)('DELETE FROM metric_levels WHERE metric_id = ?', [id]);
            for (const level of updates.applicableLevels) {
                await (0, database_1.query)('INSERT INTO metric_levels (metric_id, level) VALUES (?, ?)', [id, level]);
            }
        }
    ...
    static async deleteMetric(id) {
        if (database_1.USE_MEMORY_DB) {
            return database_1.memoryStore.performanceMetrics?.delete(id) || false;
        }
        // 先删除关联表数据
        await (0, database_1.query)('DELETE FROM scoring_criteria WHERE metric_id = ?', [id]);
        await (0, database_1.query)('DELETE FROM metric_departments WHERE metric_id = ?', [id]);
        await (0, database_1.query)('DELETE FROM metric_positions WHERE metric_id = ?', [id]);
        await (0, database_1.query)('DELETE FROM metric_levels WHERE metric_id = ?', [id]);
    ...
    static async findAllTemplates() {
        if (database_1.USE_MEMORY_DB) {
            return Array.from(database_1.memoryStore.metricTemplates?.values() || []);
        }
        const sql = `
      SELECT 
        t.*,
    ...
    // 根据岗位获取模板
    static async findTemplateByPosition(positionId) {
        if (database_1.USE_MEMORY_DB) {
            const templates = Array.from(database_1.memoryStore.metricTemplates?.values() || []);
      ...
      GROUP BY t.id
    `;
        const results = await (0, database_1.query)(sql, [positionId]);
        return results.length > 0 ? this.formatTemplate(results[0]) : null;
    }
    // 创建模板
    static async createTemplate(template) {
        const now = new Date().toISOString();
        const newTemplate = { ...template, createdAt: now, updatedAt: now };
        if (database_1.USE_MEMORY_DB) {
            if (!database_1.memoryStore.metricTemplates)
                database_1.memoryStore.metricTemplates = new Map();
            database_1.memoryStore.metricTemplates.set(template.id, newTemplate);
            return newTemplate;
        }
        const sql = `
    ...
    `;
        ...
        if (template.metrics && template.metrics.length > 0) {
            for (const metric of template.metrics) {
                await (0, database_1.query)('INSERT INTO template_metrics (template_id, metric_id, weight, required) VALUES (?, ?, ?, ?)', [template.id, metric.metricId, metric.weight, metric.required]);
            }
        }
        return newTemplate;
    }
    // ============ 辅助方法 ============
    static formatMetric(row) {
        return {
            id: row.id,
            name: row.name,
            code: row.code,
    ...
    static formatTemplate(row) {
        const metrics = [];
        if (row.metrics) {
            row.metrics.split(',').forEach((m) => {
    ...
    // 初始化默认指标
    static async initializeDefaultMetrics() {
...
exports.MetricLibraryModel = MetricLibraryModel;
//# sourceMappingURL=metricLibrary.model.js.map