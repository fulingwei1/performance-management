import { calculateTotalScore, scoreToLevel, levelToScore } from '../../utils/helpers';
...
describe('Helper Functions', () => {
  describe('calculateTotalScore', () => {
    ...
    it('should handle decimal precision correctly', () => {
      const result = calculateTotalScore(0.8, 0.9, 1.1, 1.2);
  ...
  describe('scoreToLevel', () => {
    it('should return L5 for score >= 1.4', () => {
      ...
      expect(scoreToLevel(1.45)).toBe('L5');
    ...
    it('should handle boundary values correctly', () => {
      ...
      expect(scoreToLevel(1.149)).toBe('L3');
      expect(scoreToLevel(1.15)).toBe('L4');
      
      expect(scoreToLevel(0.899)).toBe('L2');
      expect(scoreToLevel(0.9)).toBe('L3');
      
      expect(scoreToLevel(0.649)).toBe('L1');
      expect(scoreToLevel(0.65)).toBe('L2');
    });
  });
  ...
  describe('levelToScore', () => {
    ...
    it('should return 1.0 for invalid level', () => {
      expect(levelToScore('L6')).toBe(1.0);
      expect(levelToScore('')).toBe(1.0);
      expect(levelToScore('invalid')).toBe(1.0);
    });

    it('should be case sensitive', () => {
      expect(levelToScore('l5')).toBe(1.0);
      expect(levelToScore('L5')).toBe(1.5);
    });
  });
  ...
  describe('Combined score-level conversion', () => {
    it('should maintain consistency when converting score to level and back', () => {
      ...
      const level = scoreToLevel(originalScore);
      const convertedScore = levelToScore(level);
  ...
  describe('Edge cases', () => {
    ...
    it('should handle score with many decimal places', () => {
      const result = calculateTotalScore(0.555555, 1.111111, 0.999999, 1.444444);
