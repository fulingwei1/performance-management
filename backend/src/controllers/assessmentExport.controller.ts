...
export const exportMonthlyAssessments = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (normalizedMonth && !monthPattern.test(normalizedMonth)) {
    return res.status(400).json({
      success: false,
      message: '月份格式错误，请使用 YYYY-MM，例如 2026-04'
    });
  }
  ...
  const filename = `月度评分记录_${normalizedMonth || '全部'}_${new Date().getTime()}.xlsx`;
