...
export const getPendingApprovals = asyncHandler(async (req: Request, res: Response) => {
  ...
  const objectivesWithOwner = await Promise.all(
    objectives.map(async (obj) => {
      const owner = obj.ownerId ? await EmployeeModel.findById(parseInt(obj.ownerId.toString())) : null;
      return {
        ...obj,
...
export const approveObjective = [
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (objective.ownerId) {
      const employee = await EmployeeModel.findById(parseInt(objective.ownerId.toString()));
