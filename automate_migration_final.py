...
def main():
    ...
    try:
        ...
        # 2. 执行迁移
        print("\n🚀 执行 SQL 迁移...")
        cmd = f"cd {REMOTE_DIR} && docker compose exec -T postgres psql -U postgres -d performance_management -f {REMOTE_SQL}"
        ...
        # 3. 验证数据
        print("\n📊 验证数据...")
        verify_cmd = f'''cd {REMOTE_DIR} && docker compose exec -T postgres psql -U postgres -d performance_management -c "
