...
REMOTE_SQL = '/opt/performance-management/013_core_department_templates.sql'
...
def main():
    ...
    try:
        ...
        # 验证文件存在
        stdin, stdout, stderr = client.exec_command(f"ls -la {REMOTE_SQL}")
        print(f"   文件验证: {stdout.read().decode('utf-8').strip()}")
